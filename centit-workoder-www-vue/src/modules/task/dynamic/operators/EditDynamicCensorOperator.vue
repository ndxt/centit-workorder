<template>
  <ModalOperator footerHide text="处理" title="处理" :width="960">
    <TreeTransfer
      filter
      :data="all"
      :targetKeys="key"
...
<script>
  ...
  methods: {
    ...
    async queryTeam() {
      const res = await queryDynamicTeam(this.params)
    ...
    async queryEngine() {
      const res = await queryDynamicEngine(this.params)
    ...
    async queryAttention() {
      const res = await queryDynamicAttention(this.params)
    ...
    async queryOrg(type) {
      ...
      const res = await queryDynamicOrg({
        flowInstId,
        nodeInstId,
        orgExp,
